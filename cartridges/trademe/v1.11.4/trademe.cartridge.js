var IoCartridge_Trademe = (function() {
  ...
  // ══════════════════════════════════════════════════════════════
  // TRADE ME MARKETPLACE CARTRIDGE v1.11.4
  // v1.11.4 (Session 79): PIVOT trademe-listing-geo from HTML-scrape
  //   to JSON API recursive key walk. v1.11.3's debug proved TM's
  //   server returns a pure Angular SPA shell to our UA — htmlLen
  //   99342, zero JSON-LD scripts, zero TransferState blobs, zero
  //   NZ-range lat values, generic "Trade Me" <title>. The lat/lng
  //   we saw in Chrome must be client-hydrated post-boot. HTML path
  //   is dead. New path: hit /v1/Listings/{id}.json (same endpoint
  //   as trademe-listing-detail) and recursively walk the JSON for
  //   ANY key matching /lat|lon|geo|coord|location/i. If geo lives
  //   anywhere in the JSON response we'll find it here and can fold
  //   extraction into trademe-listing-detail's parser in v1.11.5.
  // v1.11.3 (Session 79): expanded HTML debug — script tag inventory,
  //   first/last 2000 chars, NZ-range number counts, state marker
  //   booleans.
  // v1.11.2 (Session 79): first HTML debug pass.
  // v1.11.1 (Session 79): jsDelivr CDN cache bust of v1.11.0.
  ...
  var modules = {
    ...
    "trademe-listing-geo": {
      ...
      category: "property",
      description: "Probes TM JSON API for any geo field anywhere in the payload",
      qualifiesWhen: ["listing_id"],
      primaryField: "listing_id",
      timeoutMs: 10000,
      // v1.11.4 PIVOT: v1.11.0-v1.11.3 scraped HTML. v1.11.3 debug
      // proved TM serves a pure Angular SPA shell to our User-Agent
      // (htmlLen ~99k, zero JSON-LD, zero TransferState, zero geo
      // field names). Pivoting to JSON API path: hit the same endpoint
      // as trademe-listing-detail but recursively scan for ANY key
      // whose name contains lat/lon/geo/coord. If the JSON API
      // exposes geo anywhere, we'll find it here.
      queryByName: function(listingId) {
        return "https://api.trademe.co.nz/v1/Listings/" + encodeURIComponent(listingId) + ".json";
      },
      fetchHeaders: {
      ...
      parse: function(data) {
        // v1.11.4 DEBUG — Recursive walk of the JSON response. Collects
        //   - top-level keys (always)
        //   - any deep key whose NAME matches /lat|lon|geo|coord|location/i
        //   - the value at that key (if primitive or short object)
        //   - count of NZ-range lat-looking and lng-looking numbers
        //     found anywhere in the serialized payload
        if (!data || typeof data !== "object") {
          return { _debug: { dataType: typeof data }, latitude: null, longitude: null, source: "debug-not-object" };
        }
        var geoKeyRe = /lat|lon|geo|coord|location/i;
        var geoHits = [];
        var topKeys = Object.keys(data);
        function walk(obj, path, depth) {
          if (depth > 6 || !obj || typeof obj !== "object") return;
          if (Array.isArray(obj)) {
            // Only walk first two items of each array — enough to spot
            // shape without drowning in list noise.
            for (var i = 0; i < Math.min(obj.length, 2); i++) {
          ...
          for (var k = 0; k < keys.length; k++) {
            ...
            if (geoKeyRe.test(key)) {
              var sample;
              if (val === null) sample = null;
              else if (typeof val === "object") {
                try { sample = JSON.stringify(val).slice(0, 300); }
                catch(e) { sample = "[unstringifiable]"; }
              } else {
                sample = val;
              }
              geoHits.push({ path: here, type: typeof val, sample: sample });
            }
            if (val && typeof val === "object") walk(val, here, depth + 1);
          }
        }
        walk(data, "", 0);
        // Serialized sweep: count NZ-range coordinate-looking numbers.
        var asText = "";
        try { asText = JSON.stringify(data); } catch(e) { asText = ""; }
        var nzLatMatches = asText.match(/-3[4-9]\.\d{3,}|-4[0-7]\.\d{3,}/g) || [];
        var nzLngMatches = asText.match(/17[2-9]\.\d{3,}|16[6-9]\.\d{3,}/g) || [];
        // If the payload happens to already include decoded numerics,
        // pull the first NZ-range lat candidate and the string 200
        // chars either side for context.
        var nzLatSample = null;
        if (nzLatMatches.length > 0) {
          var idx = asText.indexOf(nzLatMatches[0]);
          nzLatSample = {
            value: nzLatMatches[0],
            context: asText.slice(Math.max(0, idx - 200), idx + 200)
          };
        }
        // Attempt a best-effort extraction of lat/lng from the geoHits
        // array — if we find TWO hits whose paths sit in the same
        // parent and whose values are numeric in NZ range, return them.
        var pickedLat = null, pickedLng = null, pickedSource = null;
        for (var g = 0; g < geoHits.length; g++) {
          var h = geoHits[g];
          if (typeof h.sample !== "number") continue;
          var n = h.sample;
          if (/lat/i.test(h.path) && n > -48 && n < -33) {
            pickedLat = n;
          }
          if (/lon|lng/i.test(h.path) && n > 165 && n < 180) {
            pickedLng = n;
          }
        }
        if (pickedLat != null && pickedLng != null) {
          pickedSource = "trademe-jsonapi";
        }
        return {
          latitude: pickedLat,
          longitude: pickedLng,
          source: pickedSource || "debug-no-geo-in-json",
          _debug: {
            topKeys: topKeys,
            topKeyCount: topKeys.length,
            geoHitCount: geoHits.length,
            geoHits: geoHits.slice(0, 40),
            nzLatMatchCount: nzLatMatches.length,
            nzLngMatchCount: nzLngMatches.length,
            nzLatSample: nzLatSample,
            payloadByteLen: asText.length
          }
        };
      }
    },
