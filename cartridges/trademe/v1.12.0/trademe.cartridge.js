var IoCartridge_Trademe = (function() {
  ...
  function parseFrendStateFacets(html) {
    if (!html || typeof html !== "string") return { facets: null, err: "no html" };
    var match = html.match(/<script id="frend-state"[^>]*>([\s\S]*?)<\/script>/);
    if (!match) return { facets: null, err: "no frend-state script" };
    var state;
    try { state = JSON.parse(match[1]); }
    catch (e) { return { facets: null, err: "json parse: " + e.message }; }
    var search = state && state.NGRX_STATE && state.NGRX_STATE.search;
    if (!search || !search.entities) return { facets: null, err: "no NGRX search.entities" };
