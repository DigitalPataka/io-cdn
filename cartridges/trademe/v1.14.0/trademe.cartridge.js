var IoCartridge_Trademe = (function() {
  ...
  var meta = {
    id: "trademe",
    label: "Trade Me",
    version: "1.14.0",  // S83-F10: trademe-listing-detail now requests Questions array via ?return_questions=true and parses normalised questions[]. Existing questionCount field finally populates (was always 0 because TM omits Questions by default). Powers Q heat stat, QW% ratio, Quiet heat (active) signal, AND a new Q&A list rendered below the description in the listing detail panel.
    born: "Session 43",
  ...
  var modules = {
    ...
    "trademe-listing-detail": {
      ...
      queryByName: function(listingId) {
        // S83-F10: opt in to Questions via query param. TM omits the array
        // by default so the existing questionCount/Q heat stat/QW%/quiet
        // heat (active) signal all silently sat at 0. One param, same
        // single fetch, no extra round trip.
        return TM_BASE + "/Listings/" + encodeURIComponent(listingId) + ".json?return_questions=true";
      },
      fetchHeaders: TM_HEADERS,
      parse: function(data) {
