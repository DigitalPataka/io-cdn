var IoCartridge_Trademe = (function() {
  ...
  // ══════════════════════════════════════════════════════════════
  // TRADE ME MARKETPLACE CARTRIDGE v1.6.0
  ...
  var meta = {
    id: "trademe",
    label: "Trade Me",
    version: "1.9.0",  // Session 51 Solution D: sections metadata on every module — cartridge is authoritative on which section each module serves (rent/sale/jobs/reference). Mirrored in functions/index.js MODULE_SECTIONS pending a refactor that reads cartridge at resolver startup.
    born: "Session 43",
  ...
  function appendAttributeParams(params, opts, moduleId) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if (v !== null && v !== undefined && v !== "") {
        params = params + "&" + encodeURIComponent(k) + "=" + encodeURIComponent(v);
      }
    }
    return params;
  }
  ...
  var modules = {
    ...
    trademe: {
      ...
      queryByName: function(query, opts) {
        ...
        // Session 51 — Io owns the slice. Default sort = newest first
        // (ExpiryDesc) only when the user hasn't already picked a sort
        // via qualifier. Without this, TM falls back to its commercial
        // relevance order which includes promoted listings; Io should
        // know which 500 it cached and why.
