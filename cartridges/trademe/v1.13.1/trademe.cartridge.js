var IoCartridge_Trademe = (function() {
  ...
  // ══════════════════════════════════════════════════════════════
  // TRADE ME MARKETPLACE CARTRIDGE v1.13.1
  // v1.13.1 (Session 82, S82-AE): WARM AGAINST FAVICON + ABORTCONTROLLER.
  //   v1.13.0 deployed but the kit-runner started hanging on sweep+thing
  //   queries (>150s with no response). Two changes:
  //     1) warmTmSession now hits https://www.trademe.co.nz/a/favicon.ico
  //        instead of the full home page. Same-origin, so Cloudflare
  ...
  //        left the in-flight fetch dangling — likely cause of the
  //        Cloud Function hang when TM throttled or stalled the home
  //        page response.
  ...
  //        marketplace filter parameters (Region, District, Condition,
  //        Delivery, Shipping, Payment, Buy Now, Clearance) from TM
  //        website's frend-state SSR blob, with live aggregated counts.
  //     2) TM_WEB_HEADERS — full browser-shaped headers for the
  //        public website fetch (separate from the auth-gated API
  //        headers so cookies/referrers don't cross-contaminate).
  //     3) trademe.companionFetch(opts) — fires in PARALLEL with the
  //        /v1/Search/General.json API call (zero added latency for
  //        listings). Returns {facets, canonicalPath, err}.
  //     4) runModule extended — Promise.all([apiPromise, companionPromise]),
  //        merges companion.facets into parsed.data.facets. Companion
  //        failure is non-fatal: listings always render even if facets
  //        are missing.
  //   Mechanism is source-blind: any future module can declare
  //   companionFetch + return facets, the runtime handles it identically.
  //   See insight_kungfu_ssr_hydration_state.md (kung-fu Move #26).
  //   No /v1/Search/General API auth path change. Listings parser
  //   identical to v1.11.5 except now also exposes parsed.facets
  //   when companion succeeds.
  ...
  var meta = {
    id: "trademe",
    label: "Trade Me",
    version: "1.13.1",  // Session 82 (S82-AE) — WARM AGAINST FAVICON + ABORTCONTROLLER. v1.13.0 hung the kit-runner because (a) the home-page warm fetch was 99-561KB and could be throttled by TM, and (b) setTimeout resolved the outer promise but left the in-flight fetch dangling. v1.13.1 warms against /a/favicon.ico (a few KB, same-origin so cookies still mint) and wires AbortController into both warmTmSession and fetchTmSsr so timeouts actually sever the socket.
    born: "Session 43",
  ...
  // ── v1.12.0 (S82-AD): SSR Hydration State Extraction ──────────
  // TM's marketplace website (www.trademe.co.nz/a/marketplace/search)
  // embeds the full filter facet schema in a <script id="frend-state">
  // JSON blob. Parsing it gives us the 8 universal filter parameters
  // (Region, District, Condition, Delivery, Shipping, Payment, Buy Now,
  // Clearance) with live aggregated counts per query - all without
  // needing to authenticate against the gated /v1/Search/General API.
  // See insight_kungfu_ssr_hydration_state.md (kung-fu Move #26).
  // Stealth: full browser-like headers, treated as a separate fetch
  // from the API call so cookies/referrers don't cross-contaminate.
  var TM_WEB_HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
  ...
  // v1.13.1: warm against the favicon (a few KB) instead of the home
  // page (99-561KB). Same-origin so Cloudflare/TM still mint session
  // cookies on the edge. AbortController guarantees the socket closes
  // when the timeout fires — v1.13.0 leaked dangling fetches that
  // appeared to hang the kit-runner.
  var TM_WARM_URL = "https://www.trademe.co.nz/a/favicon.ico";
  ...
  function warmTmSession() {
    return new Promise(function(resolve) {
      ...
      fetch(TM_WARM_URL, opts).then(function(res) {
        clearTimeout(timer);
  ...
  // Cookied SSR fetch — inlines the network call (instead of going
  // through fetchText) so AbortController can sever the underlying
  // socket on timeout. v1.13.0 used fetchText which only resolved the
  // outer promise; the in-flight fetch leaked and could pin the Cloud
  // Function. On stripped-shell response (no NGRX search.entities),
  // the caller should _clearTmSession() so the next call rewarms.
  ...
  function parseFrendStateFacets(html) {
    ...
    var diag = { htmlLen: html.length };
    var match = html.match(/<script id="frend-state"[^>]*>([\s\S]*?)<\/script>/);
    if (!match) {
      // Diagnostic — what KIND of page did we get? Captcha? Block? Empty shell?
      diag.headSample = html.slice(0, 400);
      diag.hasCloudflareChallenge = /cloudflare|cf-ray|challenge-platform/i.test(html);
      diag.hasCaptcha = /captcha/i.test(html);
      diag.hasNoscript = /<noscript/i.test(html);
      diag.scriptCount = (html.match(/<script/gi) || []).length;
      return { facets: null, err: "no frend-state script", _diag: diag };
    }
    diag.frendStateLen = match[1].length;
    var state;
    try { state = JSON.parse(match[1]); }
    catch (e) { diag.parseErrPreview = match[1].slice(0, 200); return { facets: null, err: "json parse: " + e.message, _diag: diag }; }
    diag.topKeys = Object.keys(state || {});
    var search = state && state.NGRX_STATE && state.NGRX_STATE.search;
    if (!search || !search.entities) {
      diag.ngrxStateKeys = state.NGRX_STATE ? Object.keys(state.NGRX_STATE).slice(0, 30) : null;
      diag.searchKeys = search ? Object.keys(search) : null;
      return { facets: null, err: "no NGRX search.entities", _diag: diag };
    }
    var keys = Object.keys(search.entities);
    if (keys.length === 0) return { facets: null, err: "empty entities" };
    var item = search.entities[keys[0]] && search.entities[keys[0]].item;
    if (!item || !item.parameters) return { facets: null, err: "no item.parameters" };
    // Normalise to a clean, source-blind shape the adapter can consume
    var facets = item.parameters.map(function(p) {
      return {
        key:           p.name,
        displayName:   p.displayName,
        externalKey:   p.externalKey || p.externalOptionsKey || p.name,
        type:          p.type,
        valueType:     p.valueType,
        multiSelect:   !!p.canMultiSelect,
        defaultValue:  p.defaultValue,
        options: (p.options || []).map(function(o) {
          return { value: o.value, display: o.display, count: o.count };
        })
      };
    });
    return {
      facets:        facets,
      totalResults:  item.totalResults || item.totalCount || null,
  ...
  var modules = {
    ...
    trademe: {
      ...
      // v1.12.0 (S82-AD) — Companion SSR fetch for filter facets.
      // Runs in parallel with the API call (no extra latency for the
      // listings — they arrive when both Promises resolve). Returns
      // the universal-8 filter parameters with live aggregated counts
      // (Region, District, Condition, Delivery, Shipping, Payment,
      // Buy Now, Clearance). Per-category attributes (Model, Brand,
      // Storage Capacity for phones; Make/Year for cars; etc) come
      // from a separate harvest lookup added in Phase 5. Non-fatal:
      // if this fetch fails, listings still render — the filter panel
      // simply gets the legacy 4-filter set instead of the 8-facet set.
      companionFetch: function(opts) {
        var rawQuery = (opts && opts.raw) ? opts.raw : (opts && opts.thing) || (opts && opts.place) || "";
        if (!rawQuery) return Promise.resolve({ facets: null, err: "no query" });
        var resolved = resolvePlace(opts, true);
        var url = "https://www.trademe.co.nz/a/marketplace/search?search_string=" + encodeURIComponent(rawQuery);
  ...
  function runModule(name, url, mod, opts) {
    ...
    var apiPromise = fetcher(url, mod.timeoutMs || null, mod.fetchHeaders || null);
    // v1.12.0 (S82-AD) — companionFetch fires in PARALLEL with the API call.
    // Used by the trademe general module to extract TM website filter facets
    // from frend-state SSR. Always wrapped in .catch so a companion failure
    // can never block the primary listings response.
    var companionPromise = (mod.companionFetch ? mod.companionFetch(opts) : Promise.resolve(null))
      .catch(function(err) { return { facets: null, err: err && err.message || "companion failed" }; });
    return Promise.all([apiPromise, companionPromise]).then(function(both) {
      var data = both[0];
      var companion = both[1];
      var parsed = mod.parse(data);
      if (parsed && companion && companion.facets) {
        parsed.facets = companion.facets;
        if (companion.canonicalPath) parsed.facetCanonicalPath = companion.canonicalPath;
      } else if (parsed && companion && companion.err) {
        parsed._facetErr = companion.err;
        if (companion._diag) parsed._facetDiag = companion._diag;
      }
