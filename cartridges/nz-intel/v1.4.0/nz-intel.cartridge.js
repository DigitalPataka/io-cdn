var IoCartridge_NzIntel = (function() {
  ...
  // ══════════════════════════════════════════════════════════════
  // NZ INTELLIGENCE CARTRIDGE v1.4.0
  // Born: Session 43 (extracted from sweep v1.10.0)
  // v1.4.0 (S82-X): SEVERED realestate.co.nz entirely. Removed
  //      realestate-sale + realestate-listing-detail modules,
  //      buildRealestateHeaders, buildRealestateHtmlHeaders,
  //      parseRealestateDetailHtml, REALESTATE_REGIONS,
  //      REALESTATE_KINDS, REALESTATE_PREFS, investigateListing()
  //      and the run() route 0 listing_id branch. Strategic call:
  //      RE caps at 100/call vs TM 500/call, and TM's S82 enrichment
  //      stack (LINZ + homes + ArcGIS hazards) covers everything
  //      RE used to provide. Severability doctrine restored — TM
  //      translation table no longer carries RE fields. Git history
  //      (v1.0.0 - v1.3.0 in this folder) retains for reversal.
  ...
  // redirect:"follow" so any source's 301 redirects are handled.
  ...
  // headers (e.g. via mod.fetchHeaders) get applied per-call.
  ...
  return {
    ...
    run: function(params) {
      ...
      if (p.place) {
        ...
        // by itself), skip the Nominatim geocode entirely.
    ...
    status: function() {
      ...
      return {
        ...
        version: meta.version,
        born: meta.born,
        description: "NZ Intelligence Cartridge v1.4.0 — location sweep, seismic, weather, knowledge graph. RE severed S82-X.",
