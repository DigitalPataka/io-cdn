var IoNzIntel = function() {
  ...
  var modules = {
    ...
    trademe: {
      ...
      primaryField: "thing",
      queryByName: function(query) {
        return "https://api.trademe.co.nz/v1/Search/General.json?search_string=" + encodeURIComponent(query) + "&rows=20";
      },
      fetchHeaders: {
  ...
  function investigate(lat, lng, opts) {
    ...
    // Filter: skip "search" (geocoder), skip keyword-only modules without place
    ...
    console.log("[IoNzIntel] Investigating " + lat + ", " + lng + " | Modules: " + active.length);
    ...
    var tasks = active.map(function(name) {
      ...
      if (mod.queryByName) {
        ...
        if (queryValue) {
          url = mod.queryByName(queryValue);
        } else if (mod.query) {
    ...
    return Promise.all(tasks).then(function(results) {
      ...
      console.log("[IoNzIntel] Complete | " + ok + "/" + results.length + " modules returned data | " + response.duration_ms + "ms");
      return response;
    });
  }
  ...
  function investigateKeywordOnly(name, opts) {
    ...
    var keywordModules = Object.keys(modules).filter(function(key) {
      return key !== "search" && modules[key].queryByName;
    });

    console.log("[IoNzIntel] Keyword-only sweep for '" + name + "' | Modules: " + keywordModules.length);
    ...
    var tasks = keywordModules.map(function(key) {
      ...
      var url = mod.queryByName(queryValue);
      var headers = mod.fetchHeaders || null;
      return fetchJson(url, null, headers).then(function(data) {
    ...
    return Promise.all(tasks).then(function(results) {
      ...
      response.summary = { total: results.length, ok: ok, errors: errors, empty: results.length - ok - errors };
      console.log("[IoNzIntel] Keyword sweep complete | " + ok + "/" + results.length + " modules returned data | " + response.duration_ms + "ms");
      return response;
    });
  }
  ...
  function investigatePlace(name, opts) {
    ...
    console.log("[IoNzIntel] Resolving: " + name);

    return fetchJson(modules.search.queryByName(name)).then(function(data) {
      var parsed = modules.search.parse(data);
      if (!parsed || parsed.length === 0) {
        console.log("[IoNzIntel] Geocode failed for '" + name + "' — firing keyword modules only");
        return investigateKeywordOnly(name, options);
      }
      var best = parsed[0];
      console.log("[IoNzIntel] Resolved to: " + best.lat + ", " + best.lng);
      return investigate(best.lat, best.lng, options);
    }).catch(function(err) {
      console.log("[IoNzIntel] Geocode error: " + err.message + " — firing keyword modules only");
      return investigateKeywordOnly(name, options);
    });
  }
