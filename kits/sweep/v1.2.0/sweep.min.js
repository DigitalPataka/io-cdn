var IoNzIntel = function() {
  ...
  var modules = {
    ...
    quakes: {
      ...
      category: "seismic",
      description: "Recent earthquakes near location (within radius)",
      query: function(lat, lng) {
        return "https://api.geonet.org.nz/quake?MMI=2";
      },
      parse: function(data, lat, lng) {
        if (!data || !data.features) return null;
        var nearby = data.features.filter(function(f) {
          var c = f.geometry.coordinates;
          return Math.abs(c[1] - lat) < 1 && Math.abs(c[0] - lng) < 1;
        }).slice(0, 5);
        return {
    ...
    weather: {
      ...
      query: function(lat, lng) {
        return "https://api.open-meteo.com/v1/forecast?latitude=" + lat + "&longitude=" + lng + "&current_weather=true&timezone=Pacific%2FAuckland";
      },
      parse: function(data) {
    ...
    poi: {
      ...
      category: "local",
      description: "Schools, shops, parks, hospitals within 2km",
      query: function(lat, lng) {
        return "https://overpass-api.de/api/interpreter?data=" + encodeURIComponent("[out:json][timeout:10];(node(around:2000," + lat + "," + lng + ")[amenity];node(around:2000," + lat + "," + lng + ")[shop];);out 20;");
      },
      parse: function(data) {
        if (!data || !data.elements) return null;
        var categories = {};
        data.elements.forEach(function(el) {
          var type = el.tags.amenity || el.tags.shop || "other";
  ...
  function investigate(lat, lng, opts) {
    ...
    var tasks = active.map(function(name) {
      ...
      return fetchJson(url, null, headers).then(function(data) {
        var parsed = mod.parse(data, lat, lng);
