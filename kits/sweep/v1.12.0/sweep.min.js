var IoNzIntel = function() {
  ...
  var VERSION = "1.12.0";
  // v1.12.0 (Session 52 Phase 2): trademe-rental structured-filter dispatch.
  // queryByName now reads opts.module_attributes["trademe-rental"] populated
  // by the resolver (attribute table + place-fold patch) and emits canonical
  // TM API URL params (region, district, property_type, bedrooms_max, ...).
  // Data-gated severability: when module_attributes is absent or empty, falls
  // back to the legacy search_string path byte-identical to sweep 1.11.0.
  // Rollback = clear /contracts/modules/trademe-rental/translations/attributes
  // in Firebase RTDB (no redeploy needed). Closes the "results mismatch TM"
  // class of bug for rent queries — the ambassador "3 bedroom house rent
  // whanganui" moves from text search_string to TM-API parity structured
  // filter (expected 52 listings matching TM mirror URL exactly).
  // v1.11.0 (Session 46): Questions extraction. Parse now returns
  // questionCount (integer) from TM's Questions[] array. Feeds
  // enrichment engine QW ratio and signal detection.
  // v1.10.0 (Session 37): Listing Detail modules born. Adds
  // trademe-listing-detail (/v1/Listings/{id}.json) and trademe-similar
  // (/v1/Listings/{id}/Similar.json) plus a dedicated investigateListing()
  // entry point. Neither module auto-fires from search flows — they are
  // gated on qualifiesWhen:["listing_id"], which normal classify output
  // never emits. Reached only via investigateListing() or kit-runner
  // listing_id route. Fully severable from search.
  //
  // v1.9.0 (Session 36): preferences registry hoisted from trademe module to
  // kit-level constant so trademe-cars, trademe-bikes, trademe-boats and any
  // future price-based submodule can share one definition. Specialist parse
  // functions now surface the arbitrage fields (wasPrice, percentageOff,
  // isClearance, bidCount, isBuyNowOnly, isSuperSeller, isDealer, etc.) so
  // the shared applyPreference can actually operate on their listings.
  // Jobs / rental / flatmates remain untouched — they have different
  // preference vocabularies that will come in their own build.
  ...
  var modules = {
    ...
    trademe: {
      ...
      // v1.9.0: integrityFilter hoisted to kit-level tmIntegrityFilter.
      // The method is now a reference to the shared function so plain
      // trademe, trademe-cars, trademe-bikes and trademe-boats all share
      // one implementation.
      integrityFilter: tmIntegrityFilter,
      // v1.9.0: preference registry hoisted to kit-level TM_PREFERENCES,
      // applyPreference hoisted to kit-level tmApplyPreference. See the
      // SHARED TRADE ME PREFERENCE REGISTRY block near TM_BASE for the full
      // definition. The distinction between `qualifier` (server-side native
      // sort_order) and `preference` (client-side calculable signals) still
      // holds — only the storage location changed.
      //
  ...
  // ---- SHARED TRADE ME PREFERENCE REGISTRY (v1.9.0) ----
  // Hoisted from the plain `trademe` module so that trademe-cars, trademe-bikes
  // and trademe-boats can share one definition of the arbitrage preference
  // vocabulary. Each entry is still {synonyms, filter, sort?, describes}.
  // Jobs / rental / flatmates are intentionally NOT wired through this —
  // they'll get their own domain-specific registries in a later build.
  var TM_PREFERENCES = {
    "biggest-discount": {
  ...
  // Shared applyPreference. Pure function — no `this`. Safe to assign as a
  // method reference on any module whose parse surfaces the arbitrage fields.
  ...
  // Shared integrityFilter. Currently a size-constraint word-boundary gate
  // on listing titles. Same pure-function pattern — no `this` — so it can
  // be assigned as a method on any module that wants structural honesty.
  ...
  var tmPropertyModules = {
    "trademe-rental": {
      ...
      queryByName: function(query, opts) {
        // Session 52 Phase 2: when the resolver populates
        // opts.module_attributes["trademe-rental"], emit canonical TM API
        // URL params (region, district, property_type, bedrooms_max, ...).
        // When absent, fall through to the legacy search_string path which
        // is byte-identical to sweep 1.11.0.
        //
        // Kill switch: clear /contracts/modules/trademe-rental/translations/
        // attributes in Firebase RTDB. No redeploy needed — resolver will
        // stop populating module_attributes and this branch won't fire.
        var modAttrs = (opts && opts.module_attributes && opts.module_attributes["trademe-rental"]) || null;
        var parts = [];
        var structuredFired = false;
        if (modAttrs) {
          var attrKeys = Object.keys(modAttrs);
          ...
          if (parts.length > 0) structuredFired = true;
        }
        if (!structuredFired) {
          // Legacy mode (sweep 1.11.0 byte-identical): text search_string.
          var searchTerm = (opts && opts.place) ? opts.place : query;
          parts.push("search_string=" + encodeURIComponent(searchTerm));
        }
        if (opts && opts.qualifier) {
          ...
          if (s) parts.push("sort_order=" + s);
        }
        var rows = 20;
        if (opts && opts.scope) {
          var sc = opts.scope.toLowerCase();
          if (sc === "all" || sc === "total") rows = 50;
          if (sc === "top" || sc === "first") rows = 5;
        }
        parts.push("rows=" + rows);
        return TM_BASE + "/Search/Property/Rental.json?" + parts.join("&");
      },
    ...
    "trademe-cars": {
      ...
      fetchHeaders: TM_HEADERS,
      // v1.9.0: parse expanded to surface arbitrage fields so the shared
      // tmApplyPreference helper can operate on motor listings.
      parse: function(data) {
        if (!data || !data.List) return null;
        return {
          totalCount: data.TotalCount,
      ...
      // v1.9.0: shared hooks so motors participate in the preference pipeline.
      integrityFilter: tmIntegrityFilter,
      applyPreference: tmApplyPreference
    },
    ...
    "trademe-bikes": {
      ...
      fetchHeaders: TM_HEADERS,
      // v1.9.0: parse expanded to surface arbitrage fields.
      parse: function(data) {
        if (!data || !data.List) return null;
        return {
          totalCount: data.TotalCount,
    ...
    // Single-listing deep view via /v1/Listings/{id}.json. Public, no-auth.
    // NEVER auto-fires from investigate/investigateKeywordOnly because it has
    // no thingTriggers and no qualifiesWhen entry for normal IoQL fields.
    // Only reached via investigateListing() or the kit-runner listing_id
    // route. Severable: deleting this block strips detail view without
    // touching any search path.
    "trademe-listing-detail": {
      ...
      parse: function(data) {
        ...
        // Photos: TM can return Photos[] with {Value:{Thumbnail,Medium,Large,
        // FullSize,Gallery,Plus}} OR a flat PhotoUrls array. Normalize both.
        var photos = [];
        ...
        // Attributes — name/value pairs. Keep both raw Name and DisplayName.
        var attributes = [];
        ...
        // Member / AskingMember — either shape can appear.
        ...
        // Pickup locality — when AllowsPickups is true.
        var pickup = null;
        ...
        return {
          ...
          // Pricing — arbitrage superset (mirrors search parse)
          ...
          shipping: shipping,
          // Motor convenience hoists — some motors endpoints promote these
          // to top-level in addition to Attributes.
          year: data.Year || null,
          make: data.Make || null,
    ...
    // /v1/Listings/{id}/Similar.json. TM's own recommender — same auth
    // profile as detail. Only reached via investigateListing() or the
    // kit-runner listing_id route.
    "trademe-similar": {
      ...
      parse: function(data) {
        if (!data) return null;
        // TM returns RelatedListings wrapper OR a flat List at top level
        // OR a plain array. Normalize all three.
        var listRaw = null;
        var totalCount = null;
  ...
  // ---- LISTING DETAIL INVESTIGATION (Session 37, v1.10.0) ----
  // Dedicated entry point for fetching a single listing's full detail plus
  // its similar-listings siblings. No geocode, no search, no other modules.
  // Fires trademe-listing-detail + trademe-similar in parallel directly —
  // does NOT route through investigate/investigateKeywordOnly so we skip
  // the specialist-suppression logic and Guess Who gating.
  ...
  function investigateListing(listingId, opts) {
    ...
    if (detailMod) {
      var detailUrl = detailMod.queryByName(listingId, options);
      tasks.push(
        fetchJson(detailUrl, null, detailMod.fetchHeaders || null).then(function(data) {
          var parsed = detailMod.parse(data);
          return { module: "trademe-listing-detail", name: detailMod.name, category: detailMod.category, status: parsed ? "ok" : "empty", data: parsed };
        }).catch(function(err) {
          return { module: "trademe-listing-detail", name: detailMod.name, category: detailMod.category, status: "error", error: err.message };
        })
      );
    }

    if (similarMod) {
      var similarUrl = similarMod.queryByName(listingId, options);
      tasks.push(
        fetchJson(similarUrl, null, similarMod.fetchHeaders || null).then(function(data) {
          var parsed = similarMod.parse(data);
          return { module: "trademe-similar", name: similarMod.name, category: similarMod.category, status: parsed ? "ok" : "empty", data: parsed };
        }).catch(function(err) {
          return { module: "trademe-similar", name: similarMod.name, category: similarMod.category, status: "error", error: err.message };
        })
      );
    }
    ...
    return Promise.all(tasks).then(function(results) {
      var response = {
        query: { listing_id: listingId, place: null, lat: null, lng: null, thing: null },
        timestamp: new Date().toISOString(),
        version: VERSION,
        duration_ms: Date.now() - start,
        modules: {}
      ...
      response.summary = { total: results.length, ok: ok, errors: errors, empty: results.length - ok - errors };
      console.log("[sweep] Listing detail " + listingId + " | " + ok + "/" + results.length + " modules | " + response.duration_ms + "ms");
      return response;
    });
  }
  ...
  return {
    ...
    investigateListing: investigateListing,
    haversine: haversine,
    status: function() {
